...
(function($) {
    ...
    const SuiviInterventions = {
        ...
        bindEvents: function() {
            ...
            $('#the-list').on('click', '.quota-progress-container', this.showQuotaDetails);
            ...
            $('input[name="date_from"], input[name="date_to"]').on('change', this.validateDateRange);
        ...
        initFilters: function() {
            ...
            $('select[name="intervention_status"], select[name="projet_filter"], input[name="date_from"], input[name="date_to"]').on('change', function() {
        ...
        initValidation: function() {
            ...
            $('input[name="date_intervention"]').on('change', function() {
                ...
                if (selectedDate > today) {
                    $(this).after('<p id="date-warning" style="color: #856404; font-size: 12px; margin-top: 5px;">⚠ Cette intervention est planifiée pour le futur.</p>');
                }
            });
            ...
            $('input[name="quota"]').on('input', function() {
                ...
                if (value < 0) {
                    ...
                    $(this).after('<p id="quota-feedback" style="color: #d63384; font-size: 12px; margin-top: 5px;">⚠ Ce quota semble très élevé. Êtes-vous sûr ?</p>');
                }
            });
        },
        ...
        showQuotaDetails: function(e) {
            ...
            if ($details.length) {
                ...
                const detailsHtml = '<div class="quota-details" style="margin-top: 8px; padding: 8px; background: #f8f9fa; border-radius: 4px; font-size: 11px; border-left: 3px solid #0073aa;">' +
                    '<strong>Détails :</strong><br>' +
                    quotaText.replace(/\(|\)/g, '').replace('restant', '<br>• Restant :').replace('%', '%<br>• Pourcentage utilisé') +
                    '</div>';
        ...
        updateProgressBars: function() {
            ...
            if (document.hidden || $(document.activeElement).is('input, select, textarea')) {
                return;
            }
        ...
        validateDateRange: function() {
            ...
            if (dateFrom && dateTo && new Date(dateFrom) > new Date(dateTo)) {
                $('input[name="date_to"]').after('<p id="date-range-error" style="color: #d63384; font-size: 12px; margin-top: 5px;">⚠ La date de fin doit être postérieure à la date de début.</p>');
            }
        },
        ...
        performSearch: function() {
            ...
            $('.tablenav-pages').after('<span id="search-results-count" style="margin-left: 10px; color: #666;">(' + visibleRows + ' résultat(s) trouvé(s))</span>');
        },
        ...
        validateForms: function(e) {
            ...
            if (!isValid) {
                e.preventDefault();
                SuiviInterventions.showNotification('Veuillez remplir tous les champs requis.', 'error');
                return false;
            }
            
            // Ajouter un indicateur de chargement
            $form.find('input[type="submit"]').prop('disabled', true).after('<span class="loading-spinner"></span>');
        },
        ...
        addResetFiltersButton: function() {
            ...
            const resetButton = '<button type="button" class="button reset-filters-btn" style="margin-left: 5px;">Réinitialiser</button>';
        ...
        showNotification: function(message, type = 'info') {
            const $notification = $('<div class="notice notice-' + type + ' is-dismissible si-notification"><p>' + message + '</p></div>');
