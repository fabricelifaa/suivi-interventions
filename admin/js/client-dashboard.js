...
(function ($) {
    ...
    bindEvents: function () {
      ...
      $("#search-interventions").on("input", function () {
        ...
        searchTimeout = setTimeout(
          ClientDashboard.applyFilters.bind(ClientDashboard),
          300
        );
      });
    ...
    applyFilters: function () {
      ...
      $(".intervention-card").each(function () {
        ...
        const cardTitle = $card
          .find(".intervention-title")
          .text()
          .toLowerCase();
        const cardContent = $card
          .find(".intervention-content")
          .text()
          .toLowerCase();
  ...
  $(document).ready(function () {
    ...
    function updateSelectionCount() {
      ...
      if (checkedCount === 0) {
        $counter.html(
          '<em style="color: #d63384;">Aucun projet sélectionné - le client ne pourra voir aucune intervention.</em>'
        );
      } else {
        $counter.html(
          "<strong>" +
            checkedCount +
            "</strong> projet(s) sélectionné(s) sur <strong>" +
            totalCount +
            "</strong>"
        );
      }
    ...
    $("form").submit(function (e) {
      ...
      if (checkedCount === 0) {
        var confirm = window.confirm(
          '<?php esc_attr_e("Aucun projet n\'est sélectionné. Le client ne pourra voir aucune intervention. Voulez-vous continuer ?", "suivi-des-interventions"); ?>'
        );
        if (!confirm) {
          e.preventDefault();
          return false;
