...
<h3><?php _e('Informations Client', 'suivi-interventions'); ?></h3>

<table class="form-table">
    <tr>
        <th>
            <label for="client_company"><?php _e('Société/Organisation', 'suivi-interventions'); ?></label>
        </th>
        <td>
            <input 
                type="text" 
                ...
                class="regular-text" 
            />
            <p class="description"><?php _e('Nom de la société ou organisation du client.', 'suivi-interventions'); ?></p>
        </td>
    </tr>
    
    <tr>
        <th>
            <label for="client_contact_phone"><?php _e('Téléphone de contact', 'suivi-interventions'); ?></label>
        </th>
        <td>
            <input 
                type="tel" 
                ...
                class="regular-text" 
            />
            <p class="description"><?php _e('Numéro de téléphone de contact du client.', 'suivi-interventions'); ?></p>
        </td>
    </tr>
</table>

<h3><?php _e('Projets autorisés', 'suivi-interventions'); ?></h3>

<table class="form-table">
    <tr>
        <th>
            <label><?php _e('Projets', 'suivi-interventions'); ?></label>
        </th>
        <td>
            ...
            <?php if ($projets && !is_wp_error($projets)) : ?>
                            ...
                            <div class="projet-details">
                                <?php if ($projet_meta['quota']) : ?>
                                    <div class="projet-quota">
                                        <small>
                                            <?php 
                                            printf(
                                                __('Quota: %d interventions | Utilisé: %d (%.1f%%)', 'suivi-interventions'),
                                                $progression['quota'],
                                                $progression['used'],
                                ...
                                <?php if ($projet_meta['date_expiration']) : ?>
                                    <div class="projet-expiration">
                                        <small>
                                            <strong><?php _e('Expiration:', 'suivi-interventions'); ?></strong>
                ...
                <p class="description">
                    <?php _e('Sélectionnez les projets que ce client peut voir. Le client ne pourra consulter que les interventions liées à ces projets.', 'suivi-interventions'); ?>
                </p>
                
                <div class="client-projets-actions">
                    <button type="button" id="select-all-projets" class="button">
                        <?php _e('Tout sélectionner', 'suivi-interventions'); ?>
                    </button>
                    <button type="button" id="deselect-all-projets" class="button">
                        <?php _e('Tout désélectionner', 'suivi-interventions'); ?>
                    </button>
                </div>
                
            <?php else : ?>
                <p class="no-projets">
                    <?php _e('Aucun projet disponible.', 'suivi-interventions'); ?>
                    <a href="<?php echo admin_url('edit-tags.php?taxonomy=projet&post_type=intervention'); ?>">
                        <?php _e('Créez d\'abord des projets', 'suivi-interventions'); ?> →
                    </a>
                </p>
            <?php endif; ?>
        </td>
...
<h3><?php _e('Résumé des accès', 'suivi-interventions'); ?></h3>
<div class="client-access-summary">
    <div class="access-stats">
        ...
        <div class="stat-item">
            ...
            <span class="stat-label"><?php _e('Projet(s) autorisé(s)', 'suivi-interventions'); ?></span>
        </div>
        <div class="stat-item">
            <span class="stat-number"><?php echo $stats['total_interventions']; ?></span>
            <span class="stat-label"><?php _e('Intervention(s) visible(s)', 'suivi-interventions'); ?></span>
        </div>
        <div class="stat-item">
            <span class="stat-number"><?php echo $stats['interventions_terminees']; ?></span>
            <span class="stat-label"><?php _e('Terminée(s)', 'suivi-interventions'); ?></span>
        </div>
    </div>
</div>
<?php endif; ?>
...
jQuery(document).ready(function($) {
    ...
    $('form').submit(function(e) {
        var checkedCount = $('.projet-checkbox:checked').length;
        if (checkedCount === 0) {
            var confirm = window.confirm('<?php _e("Aucun projet n\'est sélectionné. Le client ne pourra voir aucune intervention. Voulez-vous continuer ?", "suivi-interventions"); ?>');
            if (!confirm) {
                e.preventDefault();
                return false;
            }
